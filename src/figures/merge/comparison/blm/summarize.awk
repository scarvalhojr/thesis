...

{
	if ($1 != mode || $2 != dim || $3 != window || $4 != kvalue) {
		...
		if (mode != "") {
			...
			total_time = place_time + embed_time
			printf ("%s\t%s\t%d\t%d\t%8.4f\t%8.4f\t%8.4f\t%8.3f\t%8.3f\t%8.3f\tx%d\n", mode, dim, window, kvalue, before/c, place/c, embed/c, place_time, embed_time, total_time, c)
		}
