...
middle bases. Although this is technically possible, it would require a
different implementation of OSPE in order to avoid increasing its running time
since consecutive candidates would then be unlikely to have similar prefixes. In
this implementation, the dynamic programming matrix must be computed in two
parts, from the middle rows towards the first and the last rows so that
consective candidates can be examined quickly by skipping the computation of
identical rows of the matrix.
