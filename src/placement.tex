...
conflict index minimization. With border length minimization, the generated
layout has most border conflicts concentrated between steps 7 and 58. The last
masks of this layout have low levels of border conflicts because the probes are
left-most embedded, which leaves most embeddings in an unproductive state during
the final synthesis steps. The first masks also have relatively few conflits,
and this is due to the lexicographical sorting of probes. A representation of
selected photolithographic masks resulting from this layout are shown in Fig.~
\ref{fig:greedy-bl_masks}. In this figure, it is possible to see the layers of
masked and unmasked spots in masks $M_1 \dots M_6$ that result from sorting
probes lexicographically.

With conflict index minimization, Greedy shifts border conflicts away from the
steps that synthesize the probes' middle bases (between steps 18 and 46), which
effectively reduces the average conflict index. Not surprinsingly, this
reduction comes at the expense of an increase in border length --- the
normalized border length of this particular chip rose from $17.3513$ with border
length minimization to $19.8461$ with conflict index minimization.
